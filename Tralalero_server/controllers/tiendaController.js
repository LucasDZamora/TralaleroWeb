...
exports.obtenerResumenTiendas = (req, res) => {
  const sql = `
    SELECT 
      ...
      t.nombre,
      t.valoracion,
      COUNT(r.idReseña) AS cantidadResenas
    FROM tienda t
    LEFT JOIN resenatienda r ON r.idTienda = t.idTienda
    GROUP BY t.idTienda
  `;
  ...
  db.query(sql, (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ error: 'Error al obtener resumen de tiendas' });
    }
    res.json(results);
  });
};
